...
class ForegroundLikelihood(LikelihoodBase):
    ...
    def computePower(self, ctx):
        ...
        return self.power_spectrum(visgrid, [ugrid, ugrid, eta], weights, frequencies, bins = self.n_psbins)

    def regrid(self, visibilities, baselines, frequencies, ngrid):

        umax = max([b.max() for b in baselines])*frequencies.max()

        ugrid = np.linspace(-umax, umax, ngrid)
        vis_fft2 = np.zeros((ngrid, ngrid, len(frequencies)), dtype=np.complex128)
        ...
        for j, f in enumerate(frequencies):
            ...
            im = np.histogram2d(U.flatten(), V.flatten(), bins=[ugrid, ugrid], weights=np.imag(visibilities))[0]

            vis_fft2[:,:,j] = (rl+im*1j)/weights[:,:,j]

        vis_fft2[np.isnan(vis_fft2)] = 0.0

        return ugrid, vis_fft2, weights

    def frequency_fft(self, vis, freq):
        return fft(vis, freq.max() - freq.min(), axis=-1, a = 0, b = 2*np.pi)

        #     np.bincount(pos_u, weights=FT_real)
    ...
    #         mwa_u = (self.x_displacement/lamb).value
    #         mwa_v = (self.y_displacement/lamb).value
    #
    #         vis_fft2[:,:,i], coords, weights[:,:,i] = self.sampling_regGrid(FT[:,:,i], mwa_u, mwa_v, uv_scale, cutoff = max_uv, new_cells = new_cells)
    #
    #     return vis_fft2, coords, weights
    #
    ...
    def power_spectrum(self, visibility, coords, weights, linFrequencies, bins = 100):
        ...
        PS_mK2Mpc6, k_Mpc, c_Mpc = angular_average_nd(visibility_sq, coords, bins = bins, n=3, weights=weights)
...
if __name__ == "__main__":
    ...
    from core import ForegroundCore, CoreInstrumentalSampling
