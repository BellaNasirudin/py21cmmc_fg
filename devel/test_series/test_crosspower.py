...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 11:16:02 2019
...
class CustomLikelihood(LikelihoodInstrumental2D):
    def __init__(self, n_ubins=n_ubins, uv_max=None, nrealisations=[100, 100, 3][DEBUG],
                 **kwargs):
        super().__init__(n_ubins=n_ubins, uv_max=uv_max, u_min= u_min, n_obs = n_obs, nparallel = 3,
                         simulate=True, nthreads=[20, 3, 1][DEBUG], nrealisations=nrealisations, ps_dim=2,
                         **kwargs)
...
def run_mcmc(*args, model_name, params=params, **kwargs):
    return _run_mcmc(
        *args,
        ...
        params=params,
        walkersRatio=[5, 3, 2][DEBUG],  # The number of walkers will be walkersRatio*nparams
        burninIterations=0,  # Number of iterations to save as burnin. Recommended to leave as zero.
        sampleIterations=[1000, 50, 2][DEBUG],  # Number of iterations to sample, per walker.
        threadCount=[20, 3, 1][DEBUG],  # Number of processes to use in MCMC (best as a factor of walkersRatio)
        continue_sampling=CONTINUE,  # Whether to contine sampling from previous run *up to* sampleIterations.
        **kwargs
    )
    
model_name = "crosspower_test"
...
core_fg = CorePointSourceForegrounds(S_min=50e-6, S_max=50e-3)
